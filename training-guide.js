...
export function runGuidance(run, { job = null, artifact = null, available, loading = false, busy = false, error = "", hosted = false } = {}) {
  ...
  if (status === "failed" || status === "cancelled") return result(status, status === "failed" ? "Training stopped with an error." : "Training was cancelled.", `No completed adapter was registered by this attempt. ${managed ? "The original data and partial files remain on the training Mac. " : ""}Review the details, then duplicate the recipe to make changes and try again.`, "duplicate");
  ...
  if (hosted && workflow === "managed") return managed
    ? result("hosted", "Saved history, not a live connection.", "This website cannot monitor the linked job. Open Mamase on the Mac that ran it to see its current state and files. Export this workspace only if you need to move these records; do not start another job to recover an existing one.", "backup")
    : result("hosted", "This hosted workspace cannot start training.", "Export this workspace, then open local Mamase on your Mac. In Workspace settings, choose Restore backup. Back up any existing local records first: restoring replaces them. Model files and examples are not included.", "backup");
