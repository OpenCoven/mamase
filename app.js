...
function evaluationTable(evaluations) {
  return table(["Model", "Benchmark / version", "Base → adapter", "Regressions", "Samples", "Recorded", ""], evaluations.slice().reverse().map((evaluation) => {
    ...
    return [`<a class="record-link" href="#/checkpoints/${artifact.id}">${esc(artifact.name)}</a><small>${esc(familiarContextLabel(comparison))}</small>`, `${esc(evaluation.benchmark)}<small>${comparison ? "Paired local report" : "Manual observation"}</small>`,
...
const actions = {
  ...
  "evaluation-details": (element) => {
    ...
    openModal(esc(evaluation.benchmark), `<p>${esc(evaluation.notes)}</p>${comparison ? `
      ...
      <p class="prose-notes">Familiar context: ${esc(familiarContextLabel(comparison))}</p><p class="help">Full per-case prompts, checks, and model outputs are in the original local report. These fingerprints identify imported evidence; they are not a signature or a browser verification of model files.</p>` : `<p>Score: ${evaluation.score} / ${evaluation.maximum} · ${evaluation.samples} samples. No paired base comparison was imported.</p>`}`);
  },
